...
impl AppState {
    ...
    fn todos_left(&self) -> usize {
        self.todos.get().iter().fold(0, |acc, todo| {
...
fn App<G: Html>(ctx: ScopeRef) -> View<G> {
    ...
    let todos = if let Ok(Some(app_state)) = local_storage.get_item(KEY) {
        serde_json::from_str(&app_state)
            .unwrap_or_else(|_| create_rc_signal(Vec::new()))
    } else {
...
pub fn Item<G: Html>(ctx: ScopeRef, todo: RcSignal<Todo>) -> View<G> {
    ...
    // We need a separate signal for checked because clicking the checkbox will
    // detach the binding between the attribute and the view.
    let checked = ctx.create_signal(false);
    ctx.create_effect(|| {
        // Calling checked.set will also update the `checked` property on the
        // input element.
