...
/// A router integration provides the methods for adapting a router to a certain
/// environment (e.g. history API).
...
pub struct HistoryIntegration {
    /// This field is to prevent downstream users from creating a new
    /// `HistoryIntegration` without the `new` method.
    _internal: (),
}
...
/// The sycamore router component. This component expects to be used inside a
/// browser environment. For server environments, see [`StaticRouter`].
#[component]
    ...
    I: Integration + 'static,
{
    ...
    let tmp = view.take().unwrap()(ctx, route_signal);
...
///
/// This is useful for SSR where we want the HTML to be rendered instantly
/// instead of waiting for the route preload to finish loading.
#[component]
...
/// Navigates to the specified `url`. The url should have the same origin as the
/// app.
///
/// This is useful for imperatively navigating to an url when using an anchor
/// tag (`<a>`) is not possible/suitable (e.g. when submitting a form).
///
/// # Panics
...
/// Navigates to the specified `url` without adding a new history entry.
/// Instead, this replaces the current location with the new `url`. The url
/// should have the same origin as the app.
///
/// This is useful for imperatively navigating to an url when using an anchor
/// tag (`<a>`) is not possible/suitable (e.g. when submitting a form).
///
...
mod tests {
    ...
    fn static_router() {
        ...
        assert_eq!(
            sycamore::render_to_string(
                |ctx| view! { ctx, Comp("/about".to_string()) }
            ),
            "About"
        );
        ...
        assert_eq!(
            sycamore::render_to_string(
                |ctx| view! { ctx, Comp("/404".to_string()) }
            ),
            "Not Found"
        );
    }
