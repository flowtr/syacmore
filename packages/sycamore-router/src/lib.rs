...
/// [`Route`](derive@Route) derive macro instead.
pub trait Route: Sized {
    /// Matches a route with the given path segments. Note that in general,
    /// empty segments should be filtered out before passed as an argument.
    ///
    /// It is likely that you are looking for the [`Route::match_path`] method
    /// instead.
...
pub trait TryFromParam: Sized {
    /// Creates a new value of this type from the given param. Returns `None` if
    /// the param cannot be converted into a value of this type.
    #[must_use]
...
pub trait TryFromSegments: Sized {
    ...
    /// Returns `false` if unsuccessful (e.g. parsing error).
    #[must_use]
...
mod tests {
    ...
    mod integration {
        ...
        fn router_dyn_params() {
            ...
            assert_eq!(
                Routes::match_route(&["account", "-1"]),
                Routes::NotFound
            );
            assert_eq!(
                Routes::match_route(&["account", "abc"]),
                Routes::NotFound
            );
