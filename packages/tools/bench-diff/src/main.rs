//! Tool for generating diff of js-framework-benchmark runs between two sycamore
//! versions.
...
fn main() {
    ...
    let result_str =
        fs::read_to_string(results_path).expect("could not read results file");
    let results: Vec<Result> =
        serde_json::from_str(&result_str).expect("failed to deserialize");
    let mut benchmark_results: BTreeMap<String, BenchmarkResults> =
        BTreeMap::new();
    ...
    let max_benchmark_name_len =
        benchmark_results.keys().map(|key| key.len()).max().unwrap();
