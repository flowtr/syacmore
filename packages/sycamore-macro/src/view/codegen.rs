...
//!
//! Note: we are not using the `ToTokens` trait from `quote` because we need to
//! keep track of some internal state during the entire codegen.
...
impl Codegen {
    ...
    pub fn element(&self, elem: &Element) -> TokenStream {
        ...
        let quote_attrs: TokenStream =
            attrs.iter().map(|attr| self.attribute(attr)).collect();
        ...
        let quote_children = {
            ...
            while let Some(child) = children.next() {
                ...
                if is_dyn {
                    ...
                    let marker_or_none = marker_is_some
                        .then(|| marker.clone())
                        .unwrap_or_default();

                    // If __el is a HydrateNode, use get_next_marker as initial
                    // node value.
    ...
    pub fn attribute(&self, attr: &Attribute) -> TokenStream {
        ...
        match &attr.ty {
            AttributeType::Str { name } => {
                ...
                // Use `set_class_name` instead of `set_attribute` for better
                // performance.
                ...
                {
                    // Since this is static text, intern it as it will likely be
                    // constructed many times.
                    quote! {
