...
mod hydrate_recursive {
    ...
    fn ssr() {
        check(
            &sycamore::render_to_string(v),
            expect![[
                r#"<div data-hk="0.0"><p data-hk="0.1">Nested</p></div>"#
            ]],
        );
    }
    #[wasm_bindgen_test]
...
mod top_level_fragment {
    ...
    fn ssr() {
        check(
            &sycamore::render_to_string(v),
            expect![[
                r#"<p data-hk="0.0">First</p><p data-hk="0.1">Second</p>"#
            ]],
        );
    }
    #[wasm_bindgen_test]
...
mod dynamic_template {
    ...
    fn ssr() {
        check(
            &sycamore::render_to_string(|ctx| {
                v(ctx, ctx.create_signal(view! { ctx, "text" }))
            }),
            expect![[
                r##"<p data-hk="0.0">before<!--#-->text<!--/-->after</p>"##
            ]],
        );
    }
    #[wasm_bindgen_test]
    fn test() {
        let html = sycamore::render_to_string(|ctx| {
            v(ctx, ctx.create_signal(view! { ctx, "text" }))
        });
        let c = test_container();
        c.set_inner_html(&html);
