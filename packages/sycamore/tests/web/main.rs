...
/// Returns a [`Element`] referencing the test container with the contents
/// cleared.
...
fn two_way_bind_to_props() {
    ...
    //     js_sys::Reflect::set(&input, &"value".into(),
    // &"def".into()).unwrap();     input.dispatch_event(&Event::new("input"
    // ).unwrap()).unwrap();     assert_eq!(value.get().as_str(), "def");
    // });
}
...
fn dyn_fragment_reuse_nodes() {
    create_scope_immediate(|ctx| {
        let nodes =
            vec![view! { ctx, "1" }, view! { ctx, "2" }, view! { ctx, "3" }];
        ...
        assert!(
            p.first_child()
                == nodes[0].as_node().map(|node| node.inner_element())
        );
    });
}
