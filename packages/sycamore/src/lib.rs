...
//!
//! This is the API docs for sycamore. If you are looking for the usage docs,
//! checkout the [Sycamore Book](https://sycamore-rs.netlify.app/docs/getting_started/installation).
//!
//! ## Feature Flags
//! - `dom` (_default_) - Enables rendering templates to DOM nodes. Only useful
//!   on `wasm32-unknown-unknown` target.
//! - `experimental-builder-agnostic` - Enables the agnostic backend builder
//!   API.
//! - `experimental-builder-html` - Enables the HTML specific backend builder
//!   API. Also enables `experimental-builder-agnostic`.
//! - `experimental-hydrate` - Enables client-side hydration support.
//! - `suspense` - Enables wrappers around `wasm-bindgen-futures` to make it
//!   easier to extend a reactive scope into an `async` function.
//! - `ssr` - Enables rendering templates to static strings (useful for Server
//!   Side Rendering / Pre-rendering).
//! - `serde` - Enables serializing and deserializing `Signal`s and other
//!   wrapper types using `serde`.
//! - `wasm-bindgen-interning` (_default_) - Enables interning for
//!   `wasm-bindgen` strings. This improves performance at a slight cost in
//!   binary size. If you want to minimize the size of the result `.wasm`
...
// Alias self to `sycamore` to make it possible to use proc-macros within the
// `sycamore` crate.
...
///
/// In most cases, it is idiomatic to use a glob import (aka wildcard import) at
/// the beginning of your Rust source file.
///
/// ```rust
