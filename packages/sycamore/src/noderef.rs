...
impl<G: GenericNode + Any> NodeRef<G> {
    ...
    /// [`create_node_ref`](ScopeCreateNodeRef::create_node_ref) instead.
    pub fn new() -> Self {
    ...
    /// Tries to get the T stored inside the [`NodeRef`] or `None` if it is not
    /// yet set or the wrong type.
    ///
    ...
    /// Tries to get the raw [`GenericNode`] stored inside the [`NodeRef`] or
    /// `None` if it is not yet set.
    ///
    /// For a panicking version, see [`NodeRef::get`].
