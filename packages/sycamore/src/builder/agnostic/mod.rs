    ...
    G: GenericNode,
{
    ...
    pub fn dyn_child(&self, child: impl FnMut() -> View<G> + 'a) -> &Self {
        ...
        if TypeId::of::<G>() == TypeId::of::<crate::generic_node::SsrNode>() {
            // If Server Side Rendering, insert beginning tag for hydration
            // purposes.
            self.element.append_child(&G::marker_with_text("#"));
            // Create end marker. This is needed to make sure that the node is
            ...
            render::insert(
                ...
                None,
                Some(&end_marker),
                true, /* We don't know if this is the only child or not so
                       * we pessimistically
                       * set this to true. */
            );
            return self;
        ...
        if TypeId::of::<G>() == TypeId::of::<crate::generic_node::HydrateNode>()
        {
            ...
            let el =
                <dyn Any>::downcast_ref::<crate::generic_node::HydrateNode>(
                    &self.element,
                )
                .unwrap();
            let initial = get_next_marker(&el.inner_element());
            // Do not drop the HydrateNode because it will be cast into a
            // GenericNode.
            let initial = ::std::mem::ManuallyDrop::new(initial);
            // SAFETY: This is safe because we already checked that the type is
            // HydrateNode. __initial is wrapped inside ManuallyDrop
            // to prevent double drop.
            let initial =
                unsafe { ::std::ptr::read(&initial as *const _ as *const _) };
            render::insert(
                self.ctx,
                &self.element,
                View::new_dyn(self.ctx, child),
