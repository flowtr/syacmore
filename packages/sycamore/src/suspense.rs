...
//!
//! The [`Suspense`] component is used to "suspend" execution and wait until
//! async tasks are finished before rendering.
...
pub struct SuspenseProps<'a, G: GenericNode> {
    /// The fallback [`View`] to display while the child nodes are being
    /// awaited.
    #[builder(default)]
    fallback: View<G>,
...
/// `Suspense` lets you wait for `async` tasks to complete before rendering the
/// UI. This is useful for asynchronous data-fetching or other asynchronous
/// tasks.
///
/// `Suspense` is deeply integrated with [async components](https://sycamore-rs.netlify.app/docs/basics/components).
/// Async components that are nested under the `Suspense` component will not be
/// rendered until they are resolved. Having multiple async components will have
/// the effect that the final UI will only be rendered once all individual async
/// components are rendered. This is useful for showing a loading indicator
...
pub fn Suspense<'a, G: GenericNode>(
    ...
    if let Some(outer_state) = outer_count {
        ...
        // We keep track whether outer_state has already been decremented to
        // prevent it from being decremented twice.
...
/// Creates a new "suspense scope". This scope is used to signal to a
/// [`Suspense`] component higher up in the component hierarchy that there is
/// some async task that should be awaited before rendering the UI.
///
...
impl<'a> TransitionHandle<'a> {
    /// Returns whether the transition is currently in progress or not. This
    /// value can be tracked from a listener scope.
...
/// Extension trait for [`Scope`] adding the
/// [`use_transition`](ScopeUseTransition::use_transition) method.
pub trait ScopeUseTransition<'a> {
    /// Create a new [TransitionHandle]. This allows executing updates and
    /// awaiting until all async tasks are completed.
