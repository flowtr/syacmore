...
impl<G: GenericNode> View<G> {
    ...
    /// Create a new [`View`] from a [`FnMut`] while creating a new child
    /// reactive scope.
...
pub trait IntoView<G: GenericNode> {
    /// Called during the initial render when creating the DOM nodes. Should
    /// return a `Vec` of [`GenericNode`]s.
...
impl<T: fmt::Display + 'static, G: GenericNode> IntoView<G> for T {
    fn create(&self) -> View<G> {
        ...
        // Numbers that are smaller than can be represented by an `i32` use
        // fast-path by passing value directly to JS. Note that `u16`
        // and `u32` cannot be represented by an `i32`
