...
impl SsrNode {
    ...
    ///
    /// Do not pass unsanitized user input to this function. When the node is
    /// rendered, no escaping will be performed which might lead to a XSS
...
impl GenericNode for SsrNode {
    /// Although [`SsrNode`] is intended to be used on the server-side instead
    /// of in the browser, the event type is still [`web_sys::Event`]
    /// because it must support the same API as [`DomNode`](super::DomNode).
    ...
    fn element(tag: &str) -> Self {
        ...
        if let Some(hk) = hk {
            attributes
                .insert("data-hk".to_string(), format!("{}.{}", hk.0, hk.1));
        }
    ...
    fn append_child(&self, child: &Self) {
        ...
        match self.0.ty.as_ref() {
            SsrNodeType::Element(element) => {
                element.borrow_mut().children.push(child.clone())
            }
            _ => panic!("node type cannot have children"),
        }
    }
    ...
    fn first_child(&self) -> Option<Self> {
        match self.0.ty.as_ref() {
            SsrNodeType::Element(element) => {
                element.borrow_mut().children.first().cloned()
            }
            _ => panic!("node type cannot have children"),
        }
    }
    ...
    fn update_inner_text(&self, text: &str) {
        match self.0.ty.as_ref() {
            SsrNodeType::Element(el) => {
                el.borrow_mut().children = vec![SsrNode::text_node(text)]
            }
            SsrNodeType::Comment(_c) => {
                panic!("cannot update inner text on comment node")
            }
            SsrNodeType::Text(t) => t.borrow_mut().0 = text.to_string(),
            SsrNodeType::RawText(_t) => {
                panic!("cannot update inner text on raw text node")
            }
        }
    }
    ...
    fn dangerously_set_inner_html(&self, html: &str) {
        match self.0.ty.as_ref() {
            ...
            SsrNodeType::Text(_t) => {
                panic!("cannot update inner text on text node")
            }
...
pub fn render_to_string(
    view: impl FnOnce(ScopeRef<'_>) -> View<SsrNode>,
) -> String {
...
///
/// _This API requires the following crate features to be activated: `suspense`,
/// `ssr`_
