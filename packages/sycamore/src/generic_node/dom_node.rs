...
extern "C" {
    ...
    pub fn create_text_node_int(
        this: &DocumentCreateTextNodeInt,
        num: i32,
    ) -> web_sys::Text;
}
...
impl GenericNode for DomNode {
    ...
    fn remove_property(&self, name: &str) {
        assert!(
            js_sys::Reflect::delete_property(&self.node, &name.into()).unwrap()
        );
    }
    ...
    fn insert_child_before(
        ...
        self.node
            .insert_before(
                &new_node.node,
                reference_node.map(|n| n.node.as_ref()),
            )
            .unwrap();
    }
    ...
    fn event<'a>(
        ...
        // SAFETY: extend lifetime because the closure is dropped when the ctx
        // is disposed, preventing the handler from ever being accessed
        // after its lifetime.
...
/// _This API requires the following crate features to be activated: `dom`_
pub fn render_to(
    view: impl FnOnce(ScopeRef<'_>) -> View<DomNode>,
    parent: &Node,
) {
...
/// This function is intended to be used for injecting an ephemeral sycamore
/// view into a non-sycamore app (for example, a file upload modal where you
/// want to cancel the upload if the modal is closed).
///
/// It is, however, preferable to have a single call to [`render`] or
/// [`render_to`] at the top level of your app long-term. For rendering a view
/// that will never be unmounted from the dom, use [`render_to`] instead. For
/// rendering under the `<body>` tag, use [`render`] instead.
///
/// _This API requires the following crate features to be activated: `dom`_
