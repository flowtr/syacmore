...
/// Rendering backend for the DOM with hydration support.
///
/// _This API requires the following crate features to be activated:
/// `experimental-hydrate`, `dom`_
#[derive(Clone)]
...
impl GenericNode for HydrateNode {
    ...
    /// When hydrating, instead of creating a new node, this will attempt to
    /// hydrate an existing node.
    ...
    fn append_child(&self, child: &Self) {
        if hydration_completed() {
            // Do not append nodes during hydration as that will result in
    ...
    fn insert_child_before(
        ...
        self.node.insert_child_before(
            &new_node.node,
            reference_node.map(|node| &node.node),
        );
    }
...
/// Render a [`View`] under a `parent` node by reusing existing nodes (client
/// side hydration). Alias for [`hydrate_to`] with `parent` being the `<body>`
/// tag.
///
...
/// Render a [`View`] under a `parent` node by reusing existing nodes (client
/// side hydration). For rendering under the `<body>` tag, use [`hydrate_to`]
/// instead.
///
/// For rendering without hydration, use [`render`](super::render) instead.
///
...
/// `experimental-hydrate`, `dom`_
pub fn hydrate_to(
    view: impl FnOnce(ScopeRef<'_>) -> View<HydrateNode>,
    parent: &Node,
) {
