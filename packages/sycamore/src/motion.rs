...
pub trait ScopeMotionExt<'a> {
    ...
    ///
    /// Returns a tuple of `(running, start, stop)`. The first item is a boolean
    /// signal representing whether the raf is currently running. The second
    /// item is a function to start the raf. The third item is a function to
    /// stop the raf.
    ///
    /// The raf is not started by default. Call the `start` function to initiate
    /// the raf.
    ...
    /// Does nothing if not on `wasm32` target.
    ///
    /// Instead of using `start` and `stop` functions, the callback is kept on
    /// looping until it returns `false`. `start` and `stop` are returned
    /// regardless to allow controlling the looping from outside the
    /// function.
    ///
    /// The raf is not started by default. Call the `start` function to initiate
    /// the raf.
...
impl<'a> ScopeMotionExt<'a> for Scope<'a> {
    fn create_raf(&'a self, f: impl FnMut() + 'a) -> RafState<'a> {
        ...
        if cfg!(target_arch = "wasm32") {
            ...
            // SAFETY: We are only transmuting the lifetime from 'a to 'static
            // which is safe because the closure will not be
            // accessed once the enclosing Scope is disposed.
            let extended: Box<dyn FnMut() + 'static> =
            ...
            *g.borrow_mut() = Some(Closure::wrap(Box::new({
                ...
                move || {
                    if *scope_status.get() && *running.get() {
                        // Verified that scope is still valid. We can access
                        // `extended` in here.
                        extended.borrow_mut()();
...
struct TweenedInner<'a, T: Lerp + Clone + 'a> {
    /// The [`ScopeRef`] under which the tweened signal was created. We need to
    /// hold on to the context to be able to spawn the raf callback.
    ctx: ScopeRef<'a>,
    value: RcSignal<T>,
...
impl<'a, T: Lerp + Clone + 'a> Tweened<'a, T> {
    ...
    /// Set the target value for the `Tweened`. The existing value will be
    /// interpolated to the target value with the specified
    /// `transition_duration` and `easing_fn`.
    ///
    /// If the value is being interpolated already due to a previous call to
    /// `set()`, the previous task will be canceled.
    ///
    /// To immediately set the value without interpolating the value, use
    /// `signal().set(...)` instead.
    pub fn set(&self, new_value: T) {
        ...
        let (running, start, stop) =
            self.0.borrow().ctx.create_raf_loop(move || {
                let now = Date::now();
