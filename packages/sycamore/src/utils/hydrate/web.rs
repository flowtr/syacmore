...
/// Gets the next node surrounded by `<!#>` and `<!/>`. Removes the start node
/// so that next call will return next marked nodes.
pub fn get_next_marker(parent: &Node) -> Option<View<HydrateNode>> {
    if hydration_completed() {
        ...
        // `true` if between start and end markers. Nodes that are visited when
        // start is true are added to buf.
        let mut start = false;
        ...
        for child in (0..children.length()).filter_map(|i| children.get(i)) {
            if child.node_type() == COMMENT_NODE_TYPE {
                ...
                if v.as_deref() == Some("#") {
                    ...
                    // NOTE: we can't delete the start node now because that
                    // would mess up with the node indexes.
                } else if v.as_deref() == Some("/") {
                    if start {
                        // Delete start marker. This will ensure that the next
                        // time this function is called,
                        // the same span of nodes will not be returned.
                        start_marker
                            .unwrap()
                            .unchecked_into::<Comment>()
                            .remove();
                        ...
                        buf.push(View::new_node(HydrateNode::from_web_sys(
                            child,
                        )));
                    }
                }
