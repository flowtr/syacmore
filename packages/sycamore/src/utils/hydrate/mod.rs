...
/// Run the closure inside a hydration context. If already inside a hydration
...
/// Increments the hydration component id, calls the callback, and resets the
/// component id to previous value.
///
    ...
    F: FnOnce() -> R,
{
    HYDRATION_CONTEXT.with(|context| {
        if context.borrow().is_some() {
            ...
            let r = f();
            context.borrow_mut().as_mut().unwrap().current_component_id =
                prev_component_id;
