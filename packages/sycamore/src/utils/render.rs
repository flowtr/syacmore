...
/// Insert a [`GenericNode`] under `parent` at the specified `marker`. If
/// `initial` is `Some(_)`, `initial` will be replaced with the new inserted
/// node.
///
/// # Params
/// * `parent` - The parent node to insert `accessor` under.
/// * `accessor` - The [`View`] to be inserted.
/// * `initial` - An optional initial node that is already inserted into the
///   DOM.
/// * `marker` - An optional marker node. If `marker` is `Some(_)`, `accessor`
///   will be inserted directly before `marker`. If `marker` is `None`,
///   `accessor` will be appended at the end of `parent`.
/// * `multi` - A boolean flag indicating whether the node to be inserted is the
///   only child of `parent`. Setting this to `true` will enable certain
///   optimizations when clearing the node. Even if the node to be inserted is
///   the only child of `parent`, `multi` can still be set to `false` but
///   forgoes the optimizations.
    ...
    match &value.inner {
        ViewType::Node(node) => {
            if let Some(current) = current {
                ...
                // FIXME: This is an extremely ugly hack to get around the fact
                // that current is None for text nodes when G is
                // HydrateNode. This will cause the text node to be
        ...
        ViewType::Fragment(fragment) => {
            let mut v = Vec::new();
            // normalize_incoming_fragment will subscribe to all dynamic nodes
            // in the function so as to trigger the create_effect
            // when the template changes.
            let dynamic = normalize_incoming_fragment(
                &mut v,
                fragment.as_ref(),
                unwrap_fragment,
            );
            if dynamic {
                ...
                ctx.create_effect_scoped(move |ctx| {
                    ...
                    // This will call normalize_incoming_fragment again, but
                    // this time with the unwrap_fragment
                    // arg set to true.
                    insert_expression(
                        &ctx,
                        &parent,
                    ...
                    current = Some(View::new_fragment(
                        value
                            ...
                            .map(View::new_node)
                            .collect(),
                    )); // TODO: do not perform unnecessary flattening of
                        // template
                });
            } else {
                let v = v
...
/// * `marker` - If `marker` is `None`, all the nodes from `parent` are removed
///   regardless of `current`. This behavior will likely change in the future.
...
///
/// Returns whether the normalized `Vec<G>` is dynamic (and should be rendered
/// in an effect).
///
/// # Params
/// * `v` - The [`Vec`] to write the output to.
/// * `fragment` - The `Vec<Template<G>>` to normalize.
/// * `unwrap` - If `true`, unwraps the `fragment` without setting `dynamic` to
///   true. In most cases, this should be `false`.
...
///
/// # Params
/// * `parent` - The parent node under which all other nodes are (direct)
///   children.
/// * `a` - The current/existing nodes that are to be diffed.
/// * `b` - The new nodes that are to be inserted. After the reconciliation, all
///   the nodes in `b` should be inserted under `parent`.
///
/// # Panics
...
pub fn reconcile_fragments<G: GenericNode>(parent: &G, a: &mut [G], b: &[G]) {
    ...
    while a_start < a_end || b_start < b_end {
        if a_end == a_start {
            ...
            while a_start < a_end {
                if map.is_none()
                    || !map.as_ref().unwrap().contains_key(&a[a_start])
                {
    ...
    // Sanity check: make sure all nodes in b are children of parent after
    // reconciliation.
    #[cfg(debug_assertions)]
    {
