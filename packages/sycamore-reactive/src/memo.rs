...
impl<'a> Scope<'a> {
    ...
    /// closure. If any of the tracked signals are updated, the memo is also
    /// updated.
    ///
    /// # Difference from derived signals
    ///
    /// Derived signals (functions referencing signals) are lazy and do not keep
    /// track of the result of the computation. This means that the
    /// computation will not be executed until needed. This also means that
    /// calling the derived signal twice will result in the same computation
    /// twice.
    ///
    /// ```
    ...
    /// ```
    ///
    /// Memos, on the other hand, are eagerly evaluated and will only run the
    /// computation when one of its dependencies change.
    ///
    /// Memos also incur a slightly higher performance penalty than simple
    /// derived signals.
    ///
    /// # Example
    /// ```
    ...
    /// Unlike [`create_memo`](Self::create_memo), this function will not notify
    /// dependents of a change if the output is the same. That is why the
    /// output of the function must implement [`PartialEq`].
    ///
    ...
    /// Creates a memoized value from some signals.
    /// Unlike [`create_memo`](Self::create_memo), this function will not notify
    /// dependents of a change if the output is the same.
    ///
    /// It takes a comparison function to compare the old and new value, which
    /// returns `true` if they are the same and `false` otherwise.
    ///
    /// To use the type's [`PartialEq`] implementation instead of a custom
    /// function, use [`create_selector`](Self::create_selector).
    ...
    /// An alternative to [`create_signal`](Self::create_signal) that uses a
    /// reducer to get the next value.
    ///
    ...
    /// * `reducer` - A function that takes the previous value and a message and
...
mod tests {
    ...
    fn untracked_memo() {
        create_scope_immediate(|ctx| {
            ...
            assert_eq!(*double.get(), 2);
            state.set(2);
            assert_eq!(*double.get(), 2); // double value should still be true
                                          // because state.get()
                                          // was
                                          // inside untracked
        });
    ...
    fn reducer() {
        create_scope_immediate(|ctx| {
            ...
            let (state, dispatch) =
                ctx.create_reducer(0, |state, msg: Msg| match msg {
                    Msg::Increment => *state + 1,
                    Msg::Decrement => *state - 1,
                });
