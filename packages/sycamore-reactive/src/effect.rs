...
/// The internal state of an effect. The effect callback and the effect
/// dependencies are stored in this struct.
...
impl<'a> EffectState<'a> {
    ...
    /// Should be called when re-executing an effect to recreate all
    /// dependencies.
    pub fn clear_dependencies(&mut self) {
        for dependency in &self.dependencies {
            if let Some(dep) = dependency.0.upgrade() {
                // SAFETY: We only access the pointer, not the pointed data.
                dep.unsubscribe(unsafe {
                    std::mem::transmute(Rc::as_ptr(&self.cb))
                })
            };
        }
...
impl<'a> Scope<'a> {
    ...
    /// Internal implementation for `create_effect`. Use dynamic dispatch to
    /// reduce code-bloat.
        ...
        let cb = Rc::new(RefCell::new({
            ...
            move || {
                EFFECTS.with(|effects| {
                    // Record initial effect stack length to verify that it is
                    // the same after.
                    let initial_effect_stack_len = effects.borrow().len();
                    // Upgrade the effect to an Rc now so that it is valid for
                    ...
                    // Push the effect onto the effect stack so that it is
                    // visible by signals.
                    effects.borrow_mut().push(unsafe {
                        std::mem::transmute(&mut effect as *mut EffectState<'a>)
                    });
                    // Now we can call the user-provided function.
                    f();
                    // Pop the effect from the effect stack.
                    effects.borrow_mut().pop().unwrap();
                    // The raw pointer pushed onto `effects` is dead and can no
                    // longer be accessed. We can now access
                    // `effect` directly again.

                    // For all the signals collected by the EffectState,
                    // we need to add backlinks from the signal to the effect,
                    // so that updating the signal will
                    // trigger the effect.
                    for emitter in &effect.dependencies {
                        // The SignalEmitter might have been destroyed between
                        // when the signal was
                        // accessed and now.
                        if let Some(emitter) = emitter.0.upgrade() {
                            // SAFETY: When the effect is destroyed or when the
                            // emitter is dropped,
    ...
    /// Creates an effect on signals used inside the effect closure.
    ///
    /// Instead of [`create_effect`](Self::create_effect), this function also
    /// provides a new reactive scope instead the effect closure. This scope
    /// is created for each new run of the effect.
    ///
    /// Items created within the scope cannot escape outside the effect because
    /// that can result in an use-after-free.
    ///
    /// # Example
    /// ```
    /// # use sycamore_reactive::*;
    ...
    where
        F: for<'child_lifetime> FnMut(BoundedScopeRef<'child_lifetime, 'a>)
            + 'a,
    {
        let mut disposer: Option<Box<ScopeDisposer<'a>>> = None;
        self.create_effect(move || {
            // We run the disposer inside the effect, after effect dependencies
            // have been cleared. This is to make sure that if the
            // effect subscribes to its own signal, there is no
            // use-after-free during the clear dependencies phase.
            if let Some(disposer) = disposer.take() {
                // SAFETY: we are not accessing the scope after the effect has
                // been dropped.
                unsafe {
                    disposer.dispose();
                }
            ...
            // SAFETY: transmute the lifetime. This is safe because disposer is
            // only used within the effect which is necessarily
            // within the lifetime of self (the Scope). disposer =
            // unsafe { std::mem::transmute(new_disposer) };
...
mod tests {
    ...
    fn destroy_effects_on_scope_dispose() {
        create_scope_immediate(|ctx| {
            ...
            trigger.set(());
            assert_eq!(*counter.get(), 2); // inner effect should be destroyed
