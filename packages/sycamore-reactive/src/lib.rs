...
/// Internal representation for [`Scope`]. This allows only using a single
/// top-level [`RefCell`] instead of a [`RefCell`] for every field.
#[derive(Default)]
struct ScopeInner<'a> {
...
///
/// The intended way to access a [`Scope`] is with the [`create_scope`]
/// function.
///
/// For convenience, the [`ScopeRef`] type alias is defined as a reference to a
/// [`Scope`].
///
/// # Lifetime
///
/// * `'a` - The lifetime of the scope and all data allocated on it. This allows
///   passing in data from an outer scope into an inner scope. This lifetime is
///   also invariant because it is used within an cell.
pub struct Scope<'a> {
...
impl<'a> Scope<'a> {
    /// Create a new [`Scope`]. This function is deliberately not `pub` because
    /// it should not be possible to access a [`Scope`] directly on the
    /// stack.
    pub(crate) fn new() -> Self {
        // Even though the initialization code below is same as deriving
        // Default::default(), we can't do that because accessing a raw
        // Scope outside of a scope closure breaks safety contracts.
        //
        // Self::new() is intentionally pub(crate) only to prevent end-users
        // from creating a Scope.
        Self {
...
/// A [`ScopeRef`] that is bounded by the `'bound` lifetime. This is used to
/// bypass restrictions on HRTBs (Higher Ranked Trait Bounds) so that `'a` can
/// be higher ranked while still be bounded by the `'bound` lifetime.
pub struct BoundedScopeRef<'a, 'bound: 'a>(
    ScopeRef<'a>,
    PhantomData<&'bound ()>,
);
...
impl<'a> ScopeDisposer<'a> {
    ...
    ///
    /// This method will cleanup resources in a certain order such that it is
    /// impossible to access a dangling-reference within cleanup callbacks
    /// and effects etc...
    ///
    ...
    /// # Safety
    ///
    /// `dispose` should not be called inside the `create_scope` or
    /// `create_child_scope` closure.
    ///
    /// # Drop order
    ///
    ...
    /// * `effects`
    /// * `cleanups`
    /// * `contexts` - Contexts can be refereed to inside a cleanup callback so
    ///   they are dropped after cleanups.
    /// * `arena` - Signals and refs are dropped last because they can be
    ///   refereed to in the other fields (e.g. inside a cleanup callback).
...
///
/// Returns a disposer function which will release the memory owned by the
/// [`Scope`]. Failure to call the disposer function will result in a memory
/// leak.
///
...
/// # Scope lifetime
///
/// The lifetime of the child scope is arbitrary. As such, it is impossible for
/// anything allocated in the scope to escape out of the scope because it is
/// possible for the scope lifetime to be longer than outside.
///
/// ```compile_fail
...
pub fn create_scope<'disposer>(
    ...
    // The reference passed to f cannot possible escape the closure. We know
    // however, that ptr necessary outlives the closure call because it is
    // only dropped in the returned disposer closure.
...
/// Creates a reactive scope, runs the callback, and disposes the scope
/// immediately.
///
...
pub fn create_scope_immediate(f: impl for<'a> FnOnce(ScopeRef<'a>)) {
    ...
    // SAFETY: We are not accessing the scope after calling the disposer
    // function.
    unsafe {
        disposer.dispose();
    }
...
impl<'a> Scope<'a> {
    ...
    /// The created signal lasts as long as the scope and cannot be used outside
    /// of the scope.
    ///
    /// # Signal lifetime
    ///
    ...
    /// The allocated value lasts as long as the scope and cannot be used
    ...
    ///
    /// Returns a disposer function which will release the memory owned by the
    /// [`Scope`]. If the disposer function is never called, the child scope
    /// will be disposed automatically when the parent scope is disposed.
    ///
    /// # Child scope lifetime
    ///
    /// The lifetime of the child scope is strictly a subset of the lifetime of
    /// the parent scope. ```txt
    /// [------------'a-------------]
    ///      [---------'b--------]
    /// 'a: lifetime of parent
    /// 'b: lifetime of child
    /// ```
    /// If the disposer is never called, the lifetime `'b` lasts as long as
    /// `'a`. As such, it is impossible for anything allocated in the child
    /// scope to escape into the parent scope.
    /// ```compile_fail
    /// # use sycamore_reactive::*;
    /// # create_scope_immediate(|ctx| {
    /// let mut outer = None;
    ...
    /// # });
    /// ```
    /// However, the closure itself only needs to live as long as the call to
    /// this method because it is called immediately. For example, the
    /// following compiles and is perfectly safe: ```
    /// # use sycamore_reactive::*;
    /// # create_scope_immediate(|ctx| {
        ...
        F: for<'child_lifetime> FnOnce(BoundedScopeRef<'child_lifetime, 'a>),
    {
        ...
        // SAFETY: The only fields that are accessed on self from child is
        // `context` which does not have any lifetime annotations.
        ...
        let key = self
            ...
            .borrow_mut()
            .child_scopes
            // SAFETY: None of the fields of ptr are accessed through
            // child_scopes therefore we can safely transmute the
            // lifetime.
        ...
        // - self.child_ctx is append only. That means that the Box<Ctx> will
        //   not be dropped until Self is dropped.
        f(BoundedScopeRef::new(unsafe { &*ptr }));
        //                                    ^^^ -> `ptr` is still accessible
        // here after the call to f.
        ScopeDisposer::new(move || unsafe {
            let ctx = self.inner.borrow_mut().child_scopes.remove(key).unwrap();
            // SAFETY: Safe because ptr created using Box::into_raw and closure
            // cannot live longer than 'a.
    ...
    /// However, [`dispose`](Self::dispose) only needs to take `&self` instead
    /// of `&mut self`. Dropping a [`Scope`] will automatically call
    /// [`dispose`](Self::dispose).
    pub(crate) unsafe fn dispose(&self) {
        let mut inner = self.inner.borrow_mut();
        // Drop child contexts.
        for &i in mem::take(&mut inner.child_scopes).values() {
            // SAFETY: These pointers were allocated in
    ...
    /// Returns a [`RcSignal`] that is `true` when the scope is still valid and
    /// `false` once it is disposed.
...
/// A helper function for making it explicit to define dependencies for an
/// effect.
///
/// # Params
