...
pub(crate) type SignalEmitterInner =
    RefCell<IndexMap<EffectCallbackPtr, WeakEffectCallback>>;
...
impl SignalEmitter {
    ...
    /// Adds a callback to the subscriber list. If the callback is already a
    /// subscriber, does nothing.
    pub(crate) fn subscribe(&self, cb: WeakEffectCallback) {
        self.0.borrow_mut().insert(cb.as_ptr(), cb);
    }

    /// Removes a callback from the subscriber list. If the callback is not a
    /// subscriber, does nothing.
    ...
    /// This can be useful when using patterns such as inner mutability where
    /// the state updated will not be automatically triggered. In the
    /// general case, however, it is preferable to use [`Signal::set()`]
    /// instead.
    pub fn trigger_subscribers(&self) {
        // Clone subscribers to prevent modifying list when calling callbacks.
        let subscribers = self.0.borrow().clone();
        // Subscriber order is reversed because effects attach subscribers at
        // the end of the effect scope. This will ensure that outer
        // effects re-execute before inner effects, preventing inner
        // effects from running twice.
        for subscriber in subscribers.values().rev() {
            // subscriber might have already been destroyed in the case of
            // nested effects
...
impl<T> ReadSignal<T> {
    /// Get the current value of the state. When called inside a reactive scope,
    /// calling this will add itself to the scope's dependencies.
    ///
    /// # Example
    /// ```rust
    ...
    /// Get the current value of the state, without tracking this as a
    ...
    /// When called inside a reactive scope, calling this will add itself to the
...
impl<T> Signal<T> {
    ...
    ///
    /// This will notify and update any effects and memos that depend on this
    /// value.
    ///
    /// # Example
    /// ```
    ...
    ///
    /// Make sure you know what you are doing because this can make state
    /// inconsistent.
    ...
    pub fn split(
        &self,
    ) -> (impl Fn() -> Rc<T> + Copy + '_, impl Fn(T) + Copy + '_) {
...
impl<T: Default> Signal<T> {
    ...
    /// Take the current value out and replace it with the default value
    /// _without_ triggering subscribers.
    ///
    /// Make sure you know what you are doing because this can make state
    /// inconsistent.
...
/// A trait that is implemented for all [`ReadSignal`]s regardless of the type
/// parameter.
...
///
/// Sometimes, it is useful to have a signal that can escape the enclosing
/// [reactive scope](Scope). However, this cannot be achieved simply with
/// [`Scope::create_signal`] because the resulting [`Signal`] is tied to the
/// [`Scope`] by it's lifetime. The [`Signal`] can only live as long as
/// the [`Scope`].
///
/// With [`RcSignal`] on the other hand, the lifetime is not tied to a
/// [`Scope`]. Memory is managed using a reference-counted smart pointer
/// ([`Rc`]). What this means is that [`RcSignal`] cannot implement the [`Copy`]
/// trait and therefore needs to be manually cloned into all closures where
/// it is used.
///
/// In general, [`Scope::create_signal`] should be preferred, both for
