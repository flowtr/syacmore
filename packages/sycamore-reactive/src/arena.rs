...
pub(crate) struct ScopeArena<'a> {
    inner: UnsafeCell<
        SmallVec<[*mut (dyn ReallyAny + 'a); SCOPE_ARENA_STACK_SIZE]>,
    >,
}

impl<'a> ScopeArena<'a> {
    /// Allocate a value onto the arena. Returns a reference that lasts as long
    /// as the arena itself.
    pub fn alloc<T: 'a>(&'a self, value: T) -> &'a T {
        let boxed = Box::new(value);
        let ptr = Box::into_raw(boxed);
        unsafe {
            // SAFETY: The only place where self.inner.get() is mutably borrowed
            // is right here. It is impossible to have two alloc()
            // calls on the same ScopeArena at the same time so
        ...
        // - self.inner is append only. That means that the Box<_> will not be
        //   dropped until Self is dropped.
        // - The drop code for ScopeRef never reads the allocated value and
        //   therefore does not create a stacked-borrows violation.
        unsafe { &*ptr }
    }

    /// Cleanup the resources owned by the [`ScopeArena`]. This is automatically
    /// called in [`Drop`]. However, [`dispose`](Self::dispose) only needs
    /// to take `&self` instead of `&mut self`. Dropping a [`ScopeArena`]
    /// will automatically call [`dispose`](Self::dispose).
    ///
    /// If a [`ScopeArena`] has already been disposed, calling it again does
    /// nothing.
