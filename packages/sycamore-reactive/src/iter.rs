...
impl<'a> Scope<'a> {
    /// Function that maps a `Vec` to another `Vec` via a map function. The
    /// mapped `Vec` is lazy computed, meaning that it's value will only be
    /// updated when requested. Modifications to the input `Vec` are diffed
    /// using keys to prevent recomputing values that have not changed.
    ///
    /// This function is the underlying utility behind `Keyed`.
    ///
    /// # Params
    /// * `list` - The list to be mapped. The list must be a [`ReadSignal`]
    ///   (obtained from a [`Signal`]) and therefore reactive.
    ...
    pub fn map_keyed<T, K, U>(
        ...
        map_fn: impl for<'child_lifetime> Fn(
                BoundedScopeRef<'child_lifetime, 'a>,
                T,
            ) -> U
            + 'a,
    ...
    /// mapped `Vec` is lazy computed, meaning that it's value will only be
    /// updated when requested. Modifications to the input `Vec` are diffed
    /// by index to prevent recomputing values that have not changed.
    ///
    /// Generally, it is preferred to use [`map_keyed`](Self::map_keyed) instead
    /// when a key function is available.
    ///
        ...
        self.create_effect(move || {
            ...
            if new_items.is_empty() {
                ...
                for (i, new_item) in new_items.iter().enumerate() {
                    ...
                    // We lift the equality out of the else if branch to satisfy
                    // borrow checker.
                    ...
                    if item.is_none() || eqs {
                        let new_disposer = self.create_child_scope({
                            ...
                            move |ctx| unsafe {
                                ...
                                // ptr is still accessible after
                                // self.create_child_scope and
                                // therefore lives long enough.

                                // SAFETY: f takes the same parameter as the
                                // argument to
                                // self.create_child_scope(_).
                                (*ptr).write(map_fn(
                                    mem::transmute(ctx),
                        ...
                        if item.is_none() {
                            ...
                            let prev =
                                mem::replace(&mut disposers[i], new_disposer);
                            unsafe {
                                prev.dispose();
                            }
                        }
